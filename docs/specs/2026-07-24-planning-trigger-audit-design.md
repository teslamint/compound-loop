...
observed record-shape drift. The audit section carries a single attestation
line naming the tracker state examined (file and commit) and the count of open
rows audited. No row is reproduced into the plan.
...
| Plan-schema hard floor currently has 9 items; Deferred is item 8 | `rg -n "Deferred to Follow-Up Work" schemas/plan-schema.md` | 2026-07-24T18:20:10+0900 | Single hit, line 33, numbered `8.` | Working tree at `eb93cd8` |
...
literally, without reproducing the definitional bullets.
...
- R2: `skills/planning/SKILL.md` gains a trigger-audit step that runs when
  ...
  recorded as such — never given an invented verdict.
...
- R5: `schemas/plan-schema.md`'s hard floor gains a **Carry-forward trigger
  ...
  form: one row per fired trigger (tracker row, trigger class, disposition,
  reason), plus one attestation line naming the tracker location, the tracker
  state examined (commit or equivalent), and the open-row count. When no
  trigger fired, the section is the attestation line alone. When the repo has
  no durable tracker, write exactly:
...
- R6: `skills/planning/SKILL.md` states the reviewer mandate: any independent
  ...
  fired row is a blocking finding.
...
- `rg`-decidable checks: each R1 term defined exactly once in `CONCEPTS.md`;
  ...
  `skills/planning/SKILL.md`; the hard-floor section with both exact fallback
  lines present in `schemas/plan-schema.md`.
...
- **Procedural-only reviewer mandate** — R6 is skill text, not a mechanical
  ...
  prompt discipline. Accepted deliberately: the mechanical check is deferred
  on the trigger-to-build pattern (Out of scope), and the durable audit
  section is itself the new mechanically checkable surface.

## Success Criteria

1. `CONCEPTS.md` defines edit-based trigger, drift-based trigger, event-based
   trigger, and trigger audit, each exactly once.
   - **Measured by**: `rg -c -e "edit-based trigger" -e "drift-based trigger" -e "event-based trigger" -e "trigger audit" CONCEPTS.md` returns a count ≥ 4, and a reviewer confirms each term has exactly one definitional bullet.
2. `skills/planning/SKILL.md` contains a trigger-audit step covering all
   three classes, the observability rule for drift-based triggers, and the
   exactly-one-class tiebreak.
...
4. `schemas/plan-schema.md`'s hard floor contains the Carry-forward trigger
   audit section with the lean contract and both exact fallback lines.
   - **Measured by**: `rg -n "Carry-forward trigger audit" schemas/plan-schema.md` hits the hard-floor list; `rg -n "No durable carry-forward tracker" schemas/plan-schema.md` hits the exact fallback line.
...
6. This cycle's own plan contains a conformant audit section whose
   ...
   and observed record state.
   - **Measured by**: reviewer rubric — walk ROADMAP.md's open carry-forward rows against the plan's File structure and the live `.release-loop/progress.md`; every fired row has a disposition row, every non-fired row is covered by the attestation count.
