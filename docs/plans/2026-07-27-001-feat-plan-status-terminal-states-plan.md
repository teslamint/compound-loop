...
- **`from __future__ import annotations` immediately after the module
  docstring** (a `__future__` import may be preceded only by the docstring,
  comments, and blank lines). Known Pattern:
...
- **Path fields (`superseded_by:`, `origin:`) hold repo-root-relative paths**,
  matching how the live `resume-builder` instance writes them. The validator
  derives the root itself — ascend from the plan file's directory to the nearest
  ancestor containing a `docs/` directory; fall back to the current working
  directory when no ancestor qualifies — so resolution is independent of the
  caller's CWD. (`scripts/validate.sh` is deliberately CWD-independent: it never
  `cd`s, it prefixes every path with `"$ROOT"/`; a CWD-dependent validator would
  be its first CWD-dependent check.)
...
- **Landmine (pre-existing red harness).** `scripts/test-signal-drift.sh` Case D
  ...
  lives on line 94 (Phase 7) and line 77 is blank, so Case D errors before
  testing anything — reproduced at `4cb3fdb`, before this work. U5 edits
  **Phase 8 only**, which begins at line 96 — strictly below the target — so
  the target line does **not** shift; U5's acceptance asserts it is still 94
  after the edit, and U7 records 94 as the P2 row's repair value. No unit
  cites this harness as evidence, and no unit edits it (ROADMAP row
  disposition: deferred, see trigger audit).
...
- **Writers/consumers** — `skills/planning/SKILL.md` (modify: validator gate,
  ...
  refusal).
- **Tracker** — `ROADMAP.md` (modify: four new rows, future-candidate row
...
| S3 implementing refuses terminal plan | U1 → U6 | **deliberate deviation from the code-plan integration-test form** (`schemas/plan-schema.md:30` reserves observable verification for non-code plans): U6 is a docs-only unit inside a code plan, and spec SC5 is itself a judgment rubric — the scenario evidence is a reviewer walking U6's Pre-flight text and confirming both refusals name their evidence field and neither degrades to execution |
...
Steps:
  ...
  4. Add two rejection records (spec R4), one line each, and mention each rejected value **exactly once, inside its own record** (the U1 edge acceptance depends on the count staying 1): `in-progress` — live execution state lives in commits and the progress ledger; a committed second copy is a dual source of truth that a dead session latches permanently. `abandoned` — zero observed instances; the observed need is `superseded`, which carries a successor pointer the rejected value has no slot for.
  5. Add the unknown-field rule (spec R6): consumers and the validator reject unknown `schema:` versions, never unknown fields.
  6. Add the applicability boundary (spec R8): terminal-state rules apply to plans first committed after this contract lands — keyed on the plan's creation, not its approval; no terminal state is backfilled onto earlier plans; corpus conformance is asserted for this checkout only, never for `plan/v1` as a published contract.
  7. Fold carry-forward row fixes: at line 32, change "item 1's deviation-addendum rule" to "item 3's deviation-addendum rule" (the rule lives in hard-floor item 3, Assumption Recheck); at line 33, replace "approved with this plan" with the explicit path "approved with docs/plans/2026-07-24-001-feat-planning-trigger-audit-plan.md".
  8. Commit: "feat(schema): Settle plan status enum with terminal-state evidence fields"
Acceptance: the three SC1 greps return 1/1/1; `grep -n "item 3's deviation-addendum rule" schemas/plan-schema.md` hits; `grep -n "2026-07-24-001" schemas/plan-schema.md` hits; `grep -c "first committed after" schemas/plan-schema.md` ≥ 1 (applicability boundary present).
...
Interfaces:
  Consumes: U1's contract; the helper structure of skills/compound/scripts/validate-frontmatter.py — extract_frontmatter / parse_frontmatter / check_parser_safety / check_schema / main plus usage_fail (the exit-2 producer) — with exit 0 pass, 1 validation failure with offending field named on stderr, 2 usage error
  Produces: CLI `python3 skills/planning/scripts/validate-plan-frontmatter.py <plan-path>`; path fields resolved against the root derived per Architecture notes (nearest ancestor of the plan file containing `docs/`, CWD fallback); test CLI `bash scripts/test-plan-frontmatter.sh` exiting 0 when all cases pass
Test scenarios:
...
Steps:
  1. Write scripts/test-plan-frontmatter.sh first: temp-dir setup laying fixtures under `<tmp>/docs/plans/` so the validator's root derivation lands on `<tmp>` (the `superseded_by`/`origin` targets are created inside `<tmp>`), heredoc fixtures for every scenario above, assertions on exit code and stderr substring (offending field name), summary line + nonzero exit on any case failure — follow scripts/test-signal-drift.sh's case-function style.
  2. Run it; confirm every case fails because the validator does not exist yet.
  3. Write skills/planning/scripts/validate-plan-frontmatter.py: module docstring, then `from __future__ import annotations`, then the helper structure ported from compound's validator with plan/v1 sets: REQUIRED = [schema, title, type, status, date, execution]; TYPE_VALUES = {feat, fix, refactor, chore, docs}; STATUS_VALUES = {draft, approved, done, superseded}; EXECUTION_VALUES = {code, non-code}; `schema` must equal `plan/v1`; date must match `^\d{4}-\d{2}-\d{2}$`; done ⇒ completed_by non-empty; superseded ⇒ superseded_by non-empty and resolving to a file under the derived root (Architecture notes rule); origin present ⇒ resolving likewise; keep compound's parser-safety checks; never fail on unknown keys.
...
Acceptance: `bash scripts/test-plan-frontmatter.sh` exits 0 with every case listed (spec SC2 — every invalid fixture rejected naming the field, every valid and legacy fixture accepted); the validator compiles under both boundary interpreters as resolved by the compat harness's endpoint resolution (pre-check of U3's registry gate).

## U3: harness integration — corpus check + compat registry
Execution note: skip-test-first (the corpus check is itself the test; its negative path is exercised via the scratch-copy scenario below)
Files:
  Create: (none)
...
Test scenarios:
  ...
  edge: corpus check counts the files it validated (`ok: [plan-frontmatter] <N> plans valid`, N = the glob count, ≥ 15 at write time) so the count is self-updating; an empty glob is a FAIL line, not a silent pass — a repo carrying this check with no plans is a broken assumption, not a clean state. Corpus pre-verified: all current plans carry legal `status`/`type`/`execution`, and every `origin:` value present (13 files) resolves to an existing file
...
Steps:
  ...
  2. Run `bash scripts/validate.sh`; confirm the new check passes with a validated-file count equal to the current `docs/plans/*.md` glob count (≥ 15 — the untouched corpus plus this plan file).
  3. Add one data line to the `artifact_registry()` heredoc in scripts/test-python-compatibility.sh directly below the compound validator's line, same `committed|label|path` shape: `committed|plan-frontmatter-validator|skills/planning/scripts/validate-plan-frontmatter.py`. The heredoc contains registry data, no Python — `ROADMAP.md`'s heredoc-bootstrap row ("adds or changes an inline Python heredoc") does not fire.
...
Interfaces:
  ...
  Produces: step 17 gains the validator gate following `skills/compound/SKILL.md:48`'s shape adapted for `planning`'s two commit points (step 1's text below is the literal string to write); a supersession rule; a corrected item-number reference
Test scenarios:
...
Steps:
  ...
  2. Add the supersession rule to step 17: when this plan replaces an earlier plan **first committed after the terminal-state contract landed** (`schemas/plan-schema.md` applicability boundary — plans predating it are never retroactively flipped), flip the predecessor to `status: superseded` with `superseded_by:` naming this plan's path, in the same commit that commits this plan (predecessor may be `draft` or `approved`); run the validator on the predecessor too.
...
Interfaces:
  Consumes: U1's done contract and applicability boundary; Phase 2's origin-artifact bullet at skills/retrospective/SKILL.md:34 (it presumes the plan is known and reads the plan's `origin:` to locate the spec — it does not itself discover the plan); Phase 8 "Commit & Report" (the retro-doc commit the flip rides)
  Produces: Phase 8 rule with an explicit discovery source — the plans a retro covers are: the plan path in `.release-loop/progress.md`'s `plan:` field when that ledger exists for the cycle, plus any plan path this retro's Phase 2 data or doc body cites; when neither names a plan (session-end mode), no flip. For every covered plan **first committed after the terminal-state contract landed** (U1's applicability boundary — pre-contract plans are never flipped), set `status: done` and `completed_by:` naming the commit on the base branch that landed that plan's work, in the plan's frontmatter, in the same commit as the retro doc; a retro covering multiple plans flips every qualifying one, each with its own `completed_by:`; never edit anything in the plan below the frontmatter (mutable-slot boundary, U1)
Test scenarios:
...
Steps:
  ...
  2. Assert the Case D target did not move: `grep -n "Documentation complete" skills/retrospective/SKILL.md` → 94 after the edit (U5 edits Phase 8 only, which starts below the target at line 96, so the value is expected unchanged); 94 is the repair value U7 step 4 records.
  3. Run `bash scripts/test-retro-format-drift.sh`, `bash scripts/test-signal-drift.sh`, and `bash scripts/validate.sh`. Expected: retro-format-drift green; signal-drift shows Case D red exactly as at `4cb3fdb` (pre-existing, documented in Architecture notes, not evidence for this unit) with every other case green; validate.sh exits 0.
  4. Commit: "feat(retrospective): Flip covered plans to done atomically with the retro commit"
Acceptance: greps in scenarios hit; `bash scripts/validate.sh` exits 0; step 2's grep returns 94.
...
Interfaces:
  Consumes: U1's terminal-state contract; Pre-flight items 1 (the plan read) and 2 (the contradiction scan) at skills/implementing/SKILL.md:16-17
  Produces: a Pre-flight rule scoped to the plan-file entry path ("when invoked with a plan file" — the bare-prompt entry has no frontmatter and is untouched), positioned after the plan read and before the contradiction scan: `done` → stop with a detectable error naming the recorded `completed_by:` commit ("this plan already executed; its work landed in <completed_by>"), and when a `done` plan carries no `completed_by:` (a record predating or escaping the validator), report that as a validator violation rather than inventing a commit; `superseded` → refuse, naming the `superseded_by:` successor path as where to go instead; neither terminal state ever degrades to executing the plan
Test scenarios:
  ...
  edge: the "when invoked with a plan file" scoping clause present, and the malformed-done (no `completed_by:`) branch present
...
Steps:
  1. Insert the status check as Pre-flight item 2 — after "Read the plan once", before the contradiction scan — renumbering subsequent Pre-flight items (list items, not U-IDs; the stability rule does not apply).
...
Interfaces:
  ...
  Produces: four new carry-forward/candidate rows; one candidate-row annotation; fired-row annotations for this cycle's dispositions
Test scenarios:
  happy: `grep -c "source-over-memory" ROADMAP.md` → 1; `grep -c "commit-message regeneration\|stale-message" ROADMAP.md` ≥ 1; `grep -c "post-approval body immutability\|outward publication" ROADMAP.md` ≥ 1; `grep -c "execution: ops" ROADMAP.md` → 1
...
Steps:
  ...
  2. Add four rows: (a) shipping commit-message regeneration after review — a message drafted before review is stale after review changes the artifact; trigger: next `skills/shipping/SKILL.md` edit or first stale-message incident; (b) source-over-memory cross-cutting reference — claims about file content require a same-turn read; trigger: next memory-based citation error, or next cycle touching citation rules in planning/shipping/retrospective/compound; (c) reserved designing cycle — post-approval body immutability (every section, every actor: an approved plan's §-gate was edited in place three times and the approved record erased, restorable only via byte-exact revert plus a deviation addendum) + outward publication named as a stateful ceremony (a deliverable created a public repository, pushed, force-pushed twice, and switched visibility with no mutation/failure-state matrix, because nothing named outward publication as one); trigger: before the next cycle whose deliverable crosses an outward-publication boundary, or first post-approval in-place edit in any plan/v1 repo; (d) `execution: ops` as a possible third execution mode — two instances tracked in a consuming plan/v1 repo against an enum of `code | non-code`; owner: a future `designing` cycle (spec Open Decision 2's trigger carrier); trigger: first `ops` instance in this repo, or the first request to adopt the validator in a consuming repo.
  ...
  4. Record 94 — U5 step 2's asserted, unmoved line number of the Case D target — in the red-suites (P2) row so its repair has a known value.
...
|---|---|---|---|
| Plan-schema provenance reword ("approved with this plan") | edit-based | plan list edits `schemas/plan-schema.md` | **fold** → U1 step 7 |
| Stale "item 1's deviation-addendum rule" refs (`plan-schema.md:32`, `planning/SKILL.md:120`) | edit-based | plan list edits both named files | **fold** → U1 step 7, U4 step 3 |
...
Not-fired notes recorded for auditability: the check-9/drift-harness row names check 9 specifically and `test-retro-format-drift.sh` (neither touched — U3 adds a new check elsewhere in validate.sh); the command-closure row names planning steps 13/14 and the unit template (this plan edits step 17 and the frontmatter contract); the heredoc-bootstrap row's trigger reads "adds or changes an inline **Python** heredoc" (U3 adds one data line inside the `artifact_registry()` heredoc, which contains registry rows, no Python); the python38-drift row names `skills/compound/scripts/validate-frontmatter.py` and `schemas/python-support.json` (neither touched — U2 applies its lesson to the new sibling instead).

## Deferred to Follow-Up Work

Registered as ROADMAP rows by U7, not built here:

- `review-envelope/v1` validator — remains on the "Schema validators + fixtures" row (U7 step 1).
- `execution: ops` as a possible third execution mode — U7 step 2d's row carries spec Open Decision 2's trigger; the validator flagging resume-builder's instances is intended.
...
- Shipping commit-message regeneration (U7 step 2a) and source-over-memory reference (U7 step 2b).

Discovered while planning, out of the approved spec's scope:

- `implementing`'s handling of `status: draft` plans (route back to `planning`'s USER gate) — the entry line names "an approved plan file" but prescribes nothing for a draft; surfaced by U6's design, dropped from U6 because spec SC5 covers terminal states only. Next `skills/implementing/SKILL.md` entry/pre-flight edit picks it up.

Deferred fired trigger rows (one entry per row, reasons recorded in the audit table):

- Two pre-existing red suites (P2) — repair stays with the row, now carrying the asserted target line 94.
- Pin `PYTHON_SUPPORT_FILE` in compatibility consumers — U3 touches registry data only, no delegation boundary.
- Plan internal clause-consistency mechanical check — procedural clause-diff performed by this plan's independent review instead (it returned six findings, incorporated).
- Retro-side trigger classification rule — retrospective scope this cycle is the done flip only.
- Automated numbered-reference validation — no hard-floor insertion or step renumbering this cycle.
- Carry-forward structural T-ID assertion — retro-template/check 9 untouched.
- Hand-up packet definition in shipping — shipping SKILL untouched.
- Mechanical `final_action` shape check — separate validate.sh consumer with its own row.
- Spec-level carve-out rule in designing — designing SKILL untouched.
...
- Whether the fixture temp dir uses `mktemp -d` under `TMPDIR` or the scratch convention of `test-signal-drift.sh` (`setup_copy`) — follow whichever the harness file structure makes cleaner at write time; either way fixtures sit under `<tmp>/docs/plans/` per U2 step 1.
- Boundary-interpreter paths are resolved by the compat harness's endpoint resolution; U2's compile pre-check uses what it reports.
