...
Interfaces:
  ...
  Produces: the canonical vocabularies check 9 (U5) extracts: an `Independence level:` line carrying exactly `heterogeneous | same-model fresh-context | in-thread (approximated independence) | self-checklist`, verdict forms `accepted`, `no evidenced answer (3 rejections): <verbatim>`, `self-attested`, and the seven-column table header `| ID | Round | Phase | Probe | Answer | Evidence | Verdict (verbatim) |`
Test scenarios:
...
Acceptance: `grep -c "Interview Transcript" schemas/retro-template.md` ≥ 1; the file contains `Independence level`, all four level values verbatim, `self-attested`, `no evidenced answer (3 rejections)`, `Previous doc shape`, `**Cites**`, and the `| ID | Round | Phase |` header (spec SC1).
...
Test scenarios:
  ...
  edge: level value mutated in the template propagates to a consumer mismatch `FAIL:` naming both spellings (Case B)
...
Steps:
  ...
  2. Implement check 9: locate the `## Interview Transcript` heading in the template; extract the four level values by splitting the `Independence level:` line on `|`; extract the three verdict forms from the verdict-values note; fail loudly (named `FAIL:` lines) unless exactly 4 distinct levels and 3 distinct verdict forms result; assert each level value appears verbatim in `skills/retrospective/SKILL.md`; assert `self-attested` and `no evidenced answer` appear in both `skills/retrospective/SKILL.md` and `skills/retrospective/references/interview-probes.md`; emit the `ok:` line only when zero failures.
...
No stateful ceremony in the deliverable; no mutation/failure-state matrix required. (The deliverable is skill/schema prose plus a read-only validation check; the dry runs in U6 write only to the session scratch directory and one committed evidence doc.)
