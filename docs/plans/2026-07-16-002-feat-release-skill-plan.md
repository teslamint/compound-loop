...
- In `mode:headless`, do not commit, tag, or ask a question; write
  ...
  signal.
- In `skills/release/SKILL.md`, quote each of the three canonical placeholder
  forms in inline backticks: `Release complete — v<version>`,
  `Release skipped — <reason>`, and `Release failed — <reason>`. Write every
  concrete signal instantiation as prose or inside a fenced block, never as an
  inline-backtick span, so check 6 sees coverage without treating a concrete
  reason as drift.
...
| S5 | Headless invocation prepares but never executes | U2 → U3 → U4 | U2 disposable-main smoke proves prepare-only behavior before integration; U3 drift-protects the placeholder signal contract; U4 runs the real-repo headless smoke on integrated `main` before the interactive ceremony. Covers S5. |
...
Test scenarios:
  happy: Case A leaves both manifests unmodified, derives their shared version from the disposable copy, and expects `scripts/validate.sh` exit 0 plus `ok:   plugin manifest versions agree: <copied-version>`; before check 7 exists it is red because the derived exact line is absent
...
Steps:
  ...
  2. Implement Case A without pinning a repository version: parse both manifest versions from the disposable copy, require them to agree, construct the expected check-7 success line from that copied value, and confirm red because current validation has no manifest-agreement success line.
...
Acceptance: `bash scripts/test-manifest-version-sync.sh` completes without a harness crash and exits nonzero before check 7 exists; Case A fails only on the absent success line derived from the copied manifests rather than a hardcoded release version, Cases B-D fail because validation still exits 0 or lacks the marker, Case E fails only because the marker is absent, and the run leaves no disposable-copy directory behind.
...
Files:
  ...
  Test: disposable git repositories initialized with `git init -b main`
Interfaces:
...
Test scenarios:
  ...
  integration: run `mode:headless` in a disposable main-path repo that contains the feature-worktree content on branch `main`; snapshot `git rev-parse HEAD` and `git tag --list` before and after, require an existing `.release/draft.md` whose final section contains exact commands, and require the last non-empty report line `Release skipped — headless: ceremony requires first-hand consent; draft prepared at .release/draft.md`; Covers S2, S3, S4, S5 without violating the feature-branch Preflight rule
Steps:
  1. Write `skills/release/SKILL.md` with frontmatter for `/release` and `$release`, argument parsing, Entry/Exit/Gate contract, and the seven phases Preflight, Collect, Draft, Version, Gate, Execute, Report. Inline-backtick quote each canonical placeholder signal form, but put every concrete signal example in prose or a fenced block so no concrete reason becomes a check-6 candidate.
  ...
  6. Define `mode:headless` as Preflight through Version followed by persistence of `.release/draft.md` containing provenance, inventory, drop-list, proposed version, CHANGELOG text, and exact commands; never ask, commit, or tag. State the byte-exact prepare-only skip line as prose or inside a fenced block, not an inline-backtick span.
  ...
  8. Create the four disposable git scenarios described above with `git init -b main`, copy the feature-worktree content needed by each fixture, and dispatch fresh-context agents that receive only the repo path, `skills/release/SKILL.md`, invocation mode, and expected observable state. For the interactive happy scenario, stop at the displayed USER gate; do not fabricate approval.
  9. Run the headless smoke in the disposable main-path repo. Verify HEAD and the complete tag list are byte-identical before and after, `.release/draft.md` exists, and the report's final non-empty line is exact. Do not run this smoke in the real feature worktree, where default-branch Preflight must fail by design.
...
Acceptance: all four `git init -b main` disposable scenarios produce the specified gate, provenance, failure, or skip behavior; the disposable headless smoke creates `.release/draft.md`, leaves its HEAD and tags unchanged, and ends with the exact headless skip signal; the real feature worktree is not used for headless smoke; `bash scripts/validate.sh` exits 0; the commit contains only the three named tracked files.
...
Steps:
  ...
  4. Extend check 6 in `scripts/validate.sh` through all five coordinated sites: add `release` to the producer tuple, add `skills/release/SKILL.md` to consumer files, add `Release` to the candidate state regex, map `release` to producer key `release`, change both comparisons (`len(flat) != 9` and `len(distinct) != 9`) to 12, and change the failure-message literal `exactly 9 canonical` to `exactly 12 canonical` while retaining the pairwise-distinct requirement.
...
Test scenarios:
  ...
  integration: `real-main headless smoke followed by dogfood v0.2.0 from v0.1.0..HEAD lifecycle inventory` proves prepare-only state before executing the real gate-to-commit-to-tag path and its idempotent re-run; Covers S1, S2, S4, S5
Steps:
  ...
  3. Record the pre-release HEAD, complete tag list, both manifest versions, and `git status --short`; require clean local `main`. Invoke `release mode:headless`, verify `.release/draft.md` exists and is ignored, require HEAD and the complete tag list to remain byte-identical, and require the byte-exact prepare-only skip signal as the last non-empty line.
  4. With the same clean HEAD and tags, invoke the new `release` skill interactively without an explicit version.
  5. Build the current source inventory from unique committed spec paths in `v0.1.0..HEAD`, draft `0.2.0` feature-level entries plus explicit drop reasons, draft the `0.1.0` backfill from its committed design and release retro, and present the proposal, full CHANGELOG, drop-list, and exact commands at the release USER gate.
  6. On first-hand approval, write `CHANGELOG.md`, change only each manifest's `version` value to `0.2.0`, run all three validation commands, stage exactly those three files, and verify the staged diff contains no other path before committing with message `Release v0.2.0`.
  7. Create annotated tag `v0.2.0` on the release commit with a message containing the version and approved highlights. Do not push the commit or tag.
  8. Compare the two manifest values, the newest `CHANGELOG.md` version heading, and the dereferenced newest tag; require all values to equal `0.2.0` and require `git rev-parse v0.2.0^{}` to equal HEAD.
  9. Walk every unique spec path committed in `v0.1.0..HEAD`; require its topic in the `0.2.0` CHANGELOG section or an explicit drop note. Require zero unaccounted paths, verify the annotated tag message, and emit `Release complete — v0.2.0` as the last non-empty line.
  10. Invoke `release` once more without changing the tree. Require unchanged HEAD and tag list plus final line `Release skipped — HEAD already released as v0.2.0`.
  11. Run the final integrated review over the feature integration and release commit, re-walk S1-S5 against the actual evidence, and fix no finding by rewriting the immutable release tag; any release-artifact defect requires an explicit user-approved corrective release procedure.
Acceptance: the pre-release real-main headless smoke creates only ignored `.release/draft.md`, leaves HEAD and tags unchanged, and ends with the exact prepare-only skip signal; local `main` is then clean at a release commit whose only file changes are `CHANGELOG.md` and the two manifests; annotated tag `v0.2.0` dereferences to that commit; all three validation commands exit 0 after the version bump, including Case A deriving `0.2.0` from the copied manifests; the four compared version sources equal `0.2.0`; every in-range spec is present or explicitly dropped; the interactive invocation ends `Release complete — v0.2.0`; the immediate re-run leaves HEAD and tags unchanged and ends with the exact already-released skip signal; no push or GitHub release occurs.
