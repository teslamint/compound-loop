...
8. **Deferred to Follow-Up Work** — tangential discoveries and scope creep land here, never in units (`enforces: P4`).
9. **Open unknowns** — split into *planning-time* (must resolve before approving the plan) and *implementation-time* (deferred implementation notes: exact method names, final SQL, runtime-dependent behavior — resolved during execution, listed so they are not mistaken for gaps).
