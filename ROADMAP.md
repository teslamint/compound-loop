...
| Procedural skill text can authorize durable state transitions even when the tracked diff is documentation-only | 2026-08-03 archive-on-loop-completion retro | P2 | Next planning cycle whose procedural workflow text authorizes durable local mutation — classify runtime effects and require a mutation/failure-state matrix when applicable |
