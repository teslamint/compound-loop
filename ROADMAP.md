...
| Retrospective interview protocol enforcement: the facilitator round-trip (respondent sends answers → facilitator evaluates/re-questions → up to 5 rounds) is described in `skills/retrospective/SKILL.md` Interview Protocol but not mechanically enforced — the respondent can answer internally and skip the loop, defeating the self-assessment bias guard (`enforces: P3`). Observed in dhlottery-autobuy session 001: facilitator sent 5 probes, respondent wrote answers directly into the retro doc without routing them through the facilitator for acceptance. Fix direction: either (a) the skill text must mandate a `SendMessage` round-trip with the facilitator agent before Phase 5, with a structural check that facilitator acceptance exists, or (b) restructure the interview as a workflow where the facilitator controls the write gate | 2026-07-20 dhlottery-autobuy session 001 retro (external project) | P2 | Before the next retrospective that dispatches a facilitator agent |
