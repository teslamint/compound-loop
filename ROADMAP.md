...
| Seal check in shipped validator: `validate-plan-frontmatter.py` does not verify body_seal; consuming repos get frontmatter validation but not body-seal enforcement | post-approval-immutability-and-publication-ceremony cycle (spec Open Decision 2) | P3 | First consuming-repo adoption of plan/v1 with body_seal fields |
| SC5 outward-publication rubric unexercised: the planning clause exists but no plan has been reviewed against it with the declared rubric input (stateless fallback + outward-publication unit) | post-approval-immutability-and-publication-ceremony retro | P3 | First planning cycle whose deliverable includes an outward-publication transition |
| Spec Risk mitigation traceability: Risks table mitigations are not traced to plan units via a `Covers` link; an un-traced mitigation can ship undelivered | post-approval-immutability-and-publication-ceremony retro | P3 | Next designing or planning cycle that names a Risk mitigation requiring a specific deliverable |
