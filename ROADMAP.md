...
| Locally-invoked skills can silently diverge from the repo's tracked `skills/<name>/SKILL.md`: `~/.claude/skills` → `~/.agents/skills` (dot-agents sync) holds a point-in-time snapshot, not a live view, so an in-session edit to a repo skill is invisible to skill invocations until the next sync — observed directly against `skills/retrospective/SKILL.md`, whose synced copy still had content this session had already removed from the tracked file | 2026-07-20 diff-size-metric review and planning-gate retro | P2 | Before the next skill-content edit in the same session as a later skill invocation, diff the invoked skill body against the repo's tracked file, or fix dot-agents sync to resolve live against a working checkout |
