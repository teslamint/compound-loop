...
Before rendering those values, run `git merge-base <base_branch> HEAD` and `git rev-parse HEAD`. Both full object IDs must equal `current_commit_range.base` and `.head`. A mismatch blocks measurement with `stale-commit-range`; Retro never refreshes stale authority while measuring. A current legacy ledger with no `review_counts.completeness` adopts `partial` and a fresh `counting_started_at` before structured counting starts. Render partial values as a lower bound since `counting_started_at`. An unknown `completeness` value blocks; never guess or render it.
...
Persist every facilitator round verbatim before the transcript or metrics cite it. In release-loop mode, write the exact returned bytes to a transition-owned temporary file, then use the packaged publisher from the invocation packet to publish `<artifact_root>/reviews/facilitator/round-<N>.md`. The target is `reviews/facilitator/round-<N>.md` relative to `artifact_root`; N is the dispatch ordinal. A missing, changed, conflicting, or unpublished round blocks an evidence-backed count or verdict claim. Standalone retrospective without a ledger records the same bytes in the retro document only and creates no release-loop artifact.
