...
Before every first sibling write, reject a symlink, an outside-root target, and any pre-existing filesystem or tracked target unless the existing final artifact matches this run's recorded ownership and bytes. A valid legacy active ledger may update itself at its selected path. That exemption never applies to its sibling artifact targets.
