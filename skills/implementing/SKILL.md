...
4. **Fixture evidence gate**: when the approved plan contains a Mutation/failure-state matrix, every affected unit writes one concise sanitized evidence record per applicable matrix cell before task review. Write each record to `.release-loop/evidence/U<N>/<transition-id>-<outcome>.md` and include:
