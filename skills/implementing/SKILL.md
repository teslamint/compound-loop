...
Every final artifact path below is a reserved publisher target. Write source bytes under `<artifact_root>/.tmp/` first. Then publish each temporary artifact through the packaged publisher before another consumer reads the final path.
...
1. **Brief**: write the unit's full text to `<artifact_root>/.tmp/U<N>-brief.tmp`. Include exact values, signatures, Files, Interfaces, Test scenarios, and the Execution note. Publish it to `<artifact_root>/briefs/U<N>-brief.md`. A dispatch prompt describes one unit, not the session's history — never paste prior-unit summaries into a later dispatch.
2. **Dispatch implementer**: one line on where the unit fits, the final brief path introduced as "read this first — your requirements," interfaces/decisions from earlier units the brief cannot know, your resolution of any ambiguity you noticed, and the temporary report path `<artifact_root>/.tmp/U<N>-report.tmp`. Code units follow `tdd`'s Execution note; non-code units write → self-review against spec → commit. After the worker returns, publish the temporary report to `<artifact_root>/reports/U<N>-report.md`.
3. **Status protocol**:
...
4. **Fixture evidence gate**: when the approved plan contains a Mutation/failure-state matrix, every affected unit writes one concise sanitized evidence record per applicable matrix cell before task review. Write each record to `<artifact_root>/.tmp/U<N>-<transition-id>-<outcome>.tmp`. Publish it to `<artifact_root>/evidence/U<N>/<transition-id>-<outcome>.md` and include:
...
5. **Task review**: generate a diff (`base-before-unit..HEAD`) at `<artifact_root>/.tmp/U<N>-diff.tmp`. Publish it to `<artifact_root>/reviews/U<N>-diff.txt`. Allocate the unit event through the durable review event protocol, then dispatch. Give the reviewer one approved artifact set: the approved spec, the approved plan including its Mutation/failure-state matrix, the unit evidence directory (or the plan's exact stateless fallback), every applicable committed file discovered under `docs/deviations/` whose Original contract and/or Traceability identifies that approved spec and/or plan as its source, any explicitly handed-off deviation references, the brief path, the report path, the diff path, and Global Constraints copied verbatim from the plan. The link direction is addendum -> approved source: never edit an approved spec or plan to add a backlink just so later review can find the addendum. The reviewer returns **both** verdicts — spec compliance and quality — missing either makes the review incomplete. For each integrity mechanism, the prompt requires the cheapest artifact that satisfies every written check while violating the claimed guarantee. Reviewer-prompt rules: never pre-judge or pre-rate a finding's severity, never tell the reviewer what not to flag; the constraints block is the reviewer's attention lens, copied verbatim, not paraphrased or softened.
...
9. **Fix loop**: Critical/Important findings → allocate one fix event before dispatching one fix subagent with the complete findings list. Seal its verbatim result through the durable review event protocol, then allocate a new unit re-review event. Only that verifying unit re-review may mark its source fingerprints `fixed`. **Cap: 3 rounds per unit**; on cap exhaustion, escalate to the human with the surviving findings. Minor findings → record a reasoned `deferred` disposition, do not fix now — the final branch review triages which of these must land before merge.
