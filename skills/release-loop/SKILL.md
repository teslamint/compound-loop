...
8. Require the initialize result to remain `state: new` with an empty scope. Write one complete schema-conformant record at the returned progress path. Include its validated `artifact_root` and a predicted `final_action`. Add the declaration Log line in that same and only ledger write.
