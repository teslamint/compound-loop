...
4. Create a feature branch from HEAD via `worktree-isolation` by default. Honor an explicit user request to work in the current checkout instead. Do not create a new branch or worktree when `--skip-*` resumes an existing branch.
