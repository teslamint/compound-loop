...
1. Use the executable integrity check in `progress-schema.md`. Discover records at `.release-loop/progress.md` and `.release-loop/runs/*/progress.md`. Reject each unknown schema or symlink record. Do not discard an invalid record because another record is valid.
...
2. Run the executable integrity check in `progress-schema.md`. Its archive root is the fixed `.release-loop/archive` family. Never derive the allowed root from the candidate destination. Reject absolute paths and parent escapes. Reject every symlink in each existing source or destination component.
...
4. Persist the exact collision-resolved destination in the selected ledger before any child move. For completed work, set `phase: done` and `phase_status: complete` atomically. Refresh `updated`. Log the Retro commit SHA and destination. For incomplete work, log `archived-incomplete` without a phase change.
5. A valid done record with one canonical destination marks an interrupted archive. In that case, reuse the exact recorded archive destination. Do not calculate another suffix.
6. For a scoped run, move only the remaining children of its exact `artifact_root`. Move scoped `progress.md` last as the archive commit point.
7. For a legacy run, move only `briefs`, `reports`, `reviews`, `evidence`, and its corrupt backups. Move the selected root `progress.md` last.
8. Verify the terminal record against the retained destination. A rerun reads the destination from the source ledger. It moves only remaining children. It never reverses a terminal archive.
