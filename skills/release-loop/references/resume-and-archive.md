...
5. Run `python3 "$release_loop_skill_root/scripts/run-artifact-integrity.py" archive --repo . --progress-path <repo-relative-progress-path> --destination <repo-relative-archive-path>` only after step 4 persists the canonical evidence. The CLI validates that exact marker and phase state before a move. It never writes ledger evidence. On rerun, omit `--destination`; the command reads the persisted path.
