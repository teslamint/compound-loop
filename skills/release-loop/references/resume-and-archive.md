...
1. Discover valid live records at `.release-loop/progress.md` and `.release-loop/runs/*/progress.md`. Reject unknown `schema:` versions rather than guessing.
...
2. Reject absolute paths and parent escapes. Reject every symlink in each existing source or destination component. Verify physical parents remain inside the selected active root or exact terminal archive root.
...
4. Persist the exact collision-resolved destination before any move. For completed work, set `phase: done` and `phase_status: complete` atomically. Refresh `updated`. Log the Retro commit SHA and destination. For incomplete work, log `archived-incomplete` without a phase change.
...
8. Verify the terminal record against the retained destination. A rerun moves only remaining children. It never reverses a terminal archive.

The archive transition is local-only and can run headlessly after it proves that no outward target exists. Pre-move cancellation preserves the source and recorded destination. Mid-move cancellation leaves the selected progress record in the source scope. The next invocation finishes the same destination. Ambiguous bytes require manual recovery without source deletion.
