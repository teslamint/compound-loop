...
- **When deepening an already-approved plan (interactive mode), recompute `body_seal` per the canonical extraction in the planning skill's local `schemas/plan-schema.md` "Body seal" and write the new digest in the same commit as the body changes. That same commit message MUST contain the exact first-hand authorization marker `interactive-deepening=first-hand-explicit`; this is the sole authorized re-seal path. No other editing path may update `body_seal` after the initial approval.
