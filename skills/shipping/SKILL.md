...
When the selected ledger is a legacy record (`artifact_root: .release-loop`), permit `.release-loop` removal only after the handoff CLI result records `cleanup_permitted: true` for a complete marker at `.release-loop/.handoff/<feature>.json` and an exact base discovery of `.release-loop/progress.md`. Cancellation or any non-complete handoff result preserves both `.release-loop` roots -- source worktree and base checkout -- and blocks local worktree cleanup until it completes.
