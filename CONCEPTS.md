...
- **Gateway micro-USD** — a conservative pre-spend accounting unit derived from governed token-price ceilings; it does not claim to reproduce a provider invoice.
- **Last-mile admission** — a request gate that runs after final model, route, auth, header, and body construction but before any network bytes leave.
